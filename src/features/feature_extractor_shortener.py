...
## LANGKAH 1: TAMBAHKAN POLA INI ##
SHORTENER_PATTERN = re.compile(
    r'\b(1url\.com|adf\.ly|bc\.vc|bit\.do|bit\.ly|bitly\.com|bkite\.com|bl\.ink|'
    r'budurl\.com|buzurl\.com|cli\.gs|cur\.lv|cutt\.ly|cutt\.us|db\.tt|doiop\.com|'
    r'ff\.im|fic\.kr|filoops\.info|go2l\.ink|goo\.gl|is\.gd|ity\.im|j\.mp|'
    r'just\.as|kl\.am|link\.zip\.net|lnkd\.in|loopt\.us|migre\.me|om\.ly|ow\.ly|'
    r'ping\.fm|po\.st|post\.ly|prettylinkpro\.com|q\.gs|qr\.ae|qr\.net|'
    r'rebrand\.ly|rubyurl\.com|short\.ie|short\.io|short\.to|shorte\.st|'
    r'snip\.ly|snipr\.com|snipurl\.com|scrnch\.me|su\.pr|t\.co|t2m\.io|'
    r'tiny\.cc|tinyurl\.com|to\.ly|tr\.im|tweez\.me|twit\.ac|twitthis\.com|'
    r'twurl\.nl|u\.bb|u\.to|url4\.eu|v\.gd|vzturl\.com|wp\.me|x\.co|yfrog\.com|'
    r'yourls\.org)\b',
    re.IGNORECASE
)
...
def normalize_url(url_string):
    """
    Membersihkan dan menormalisasi string URL dari berbagai kesalahan input umum.
    """
    # Menangani input yang bukan string (misal: None atau float)
    if not isinstance(url_string, str):
        return "" 
    
    # 1. Hapus spasi di awal/akhir dan ubah ke huruf kecil untuk konsistensi
    url = url_string.strip()
    
    # 2. Ganti semua backslash (\) dengan forward slash (/)
    url = url.replace('\\', '/')
    
    # 3. Deteksi dan perbaiki skema yang salah (misal: http:/, https:/, htpps://, htp://, dll.)
    # Pola ini mencari 'http', 'https', 'htp', 'htpps', dll. diikuti oleh ':/' atau '://'
    match = re.match(r'^(ht{1,2}ps?):?/{1,}', url, re.IGNORECASE)
    
    if match:
        # Ambil sisa URL setelah bagian skema yang salah
        rest_of_url = url[len(match.group(0)):]
        
        # Tentukan skema yang benar, prioritaskan 'https' jika ada 's'
        scheme = "https://" if "s" in match.group(1).lower() else "http://"
        
        # Gabungkan kembali menjadi URL yang benar
        url = scheme + rest_of_url
    
    # 4. Jika setelah semua perbaikan masih tidak ada skema, tambahkan https://
...
def extract_features(url_input):
    ...
    ## NORMALISASI BARU YANG LEBIH KUAT ##
    # Semua logika pembersihan URL yang rumit dipindahkan ke fungsi normalize_url
    ...
    logger.info(f"Mengecek apakah {url_input} adalah URL shortener.")
    headers = {
    ...
    # Cek sekali apakah URL adalah shortener dan simpan hasilnya di variabel 'is_shortener'
    is_shortener = bool(SHORTENER_PATTERN.search(url_input)) 
    
    if is_shortener:
        logger.debug(f"URL Shortener terdeteksi, mencoba mendapatkan URL final dari: {url_input}")
        try:
            # Gunakan requests.head untuk mengikuti pengalihan
            head_resp = requests.head(current_url, headers=headers, allow_redirects=True, timeout=10)
            final_url = head_resp.url
            
            # Perbarui 'current_url' hanya jika URL-nya berubah
            if final_url != current_url:
                 logger.info(f"URL berhasil diperluas dari {current_url} menjadi {final_url}")
                 current_url = final_url 
            else:
                 logger.debug("URL shortener mengarah ke URL yang sama atau gagal diperluas.")

        except requests.exceptions.RequestException as e:
            logger.warning(f"Gagal memperluas URL shortener {url_input}: {e}. Melanjutkan dengan URL asli.")
            pass # Jika gagal, biarkan saja dan lanjutkan dengan URL yang ada
